import { type AnyNodeId, emitter, type GridEvent, ShearWallNode, useScene } from '@pascal-app/core'
...
/**
 * ShearWallTool — polygon drawing tool for creating shear wall panels.
 * Click points to define a polygon, close the polygon to create a shear wall.
 */
export const ShearWallTool: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const commitShearWallDrawing = (points: Array<[number, number]>) => {
      const { createNode, nodes } = useScene.getState()

      const shearWallCount = Object.values(nodes).filter((n) => n.type === 'shear-wall').length
      const name = `Shear Wall ${shearWallCount + 1}`

      const node = ShearWallNode.parse({
        name,
        points,
        sheathingType: 'osb',
        nailingSchedule: '8d @ 6/12',
      })
    ...
    const onGridClick = (event: GridEvent) => {
      ...
      if (
        ...
        commitShearWallDrawing(pointsRef.current)
